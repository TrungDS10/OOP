...
public class Main {
    ...
    private void loadWordsIntoTable() {
        ...
        for (Product product : products.getListProduct()) {
            ...
            sb.append("<html><pre>");
            sb.append(String.format("%d.\t%-15s\t%d\t%-20s", i++, product.getProductName(), product.getAmount(),
    ...
    public Main() {
        try {
            products = JSONS_IO.loadJSONFile();
    ...
    public void initialize() {
        ...
        Object[] data = products.getListProduct().toArray();
        list = new JList(data);
        ...
        /**
         * Input product name or choose item from JList, then click REMOVE button to
        ...
        removeButton.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if (textField_productName.getText().length() > 0) {
                    for (Product product : products.getListProduct()) {
                        if (textField_productName.getText().toUpperCase().equals(product.getProductName().toUpperCase())
                                && !product.decreaseAmount()) {
                            toDeleteProduct = product;
                        }
                    }
                    products.getListProduct().remove(toDeleteProduct);
                    loadWordsIntoTable();
                } else if (list.getSelectedIndex() != -1) {
                    Product product = products.getListProduct().get(list.getSelectedIndex());
                    if (!product.decreaseAmount()) {
                        products.getListProduct().remove(product);
                    }
                    loadWordsIntoTable();
